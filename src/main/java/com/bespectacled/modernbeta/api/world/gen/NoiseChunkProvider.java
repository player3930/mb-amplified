...
public abstract class NoiseChunkProvider extends BaseChunkProvider {
    ...
    private void generateTerrain(Chunk chunk, StructureAccessor structureAccessor) {
        ...
        for (int subChunkX = 0; subChunkX < this.noiseSizeX; ++subChunkX) {
		    ...
		    boolean wholeSubY = false; // track whether value of subY ends in .0 or .5

                    for(float subY = 0; subY < this.verticalNoiseResolution - 0.2; subY += 0.5) {

			wholeSubY = !wholeSubY; // every other iteration the value of subY ends in .5
			if(wholeSubY)
				subY = (float) Math.round(subY); // to keep precision
			float y2 = subY + subChunkY * this.verticalNoiseResolution;
                        y2 += this.worldMinY;

			int y = (int) Math.round(y2);

			if(!wholeSubY && y < 64)
				continue; // don't double the iterations for blocks below sea level
				...
				if(y2 > 64)
					y = (int) (Math.round(2 * (y2 - 64))) + 64; // double y's distance above sea level
     ...
     */
    private BlockSource getBaseBlockSource(NoiseProvider baseNoiseProvider, StructureWeightSampler weightSampler) {
        return (x, y, z) -> {
            double density = baseNoiseProvider.sample();
