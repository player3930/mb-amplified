...
public class BetaChunkProvider extends NoiseChunkProvider {
	...
	scale /= 2;
		...
		int cap = this.worldTopY - 1;
		if(cap > 64) { // changed from 64
			cap = (cap - 64) * 2 + 65;
		}
                for (int y = cap; y >= this.worldMinY; y--) { // changed from "this.worldTopY - 1"
                    ...
                    if (runDepth == -1) {
			int beachCap = this.seaLevel + 1;
			if(beachCap > 64) {
				beachCap = (beachCap - 64) * 2 + 64;
			}
                        if (surfaceDepth <= 0) { // Generate stone basin if noise permits
                            topBlock = BlockStates.AIR;
                            fillerBlock = this.defaultBlock;
                            
                        } else if (y >= this.seaLevel - 4 && y <= beachCap) { // Generate beaches at this y range
    ...
    protected void sampleNoiseColumn(double[] buffer, int startNoiseX, int startNoiseZ, int localNoiseX, int localNoiseZ) {
        ...
        double coordinateScale = 342.206D * this.xzScale; 
	...
	double rain = clime.rain() * temp;
        ...
        double scale = this.scaleNoiseOctaves.sampleXZ(noiseX, noiseZ, 0.5605D, 0.5605D);
